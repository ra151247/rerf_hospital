...
export default class FacilityUsedAdmin extends React.Component{
    ...
    statusArrpove(id){
        ...
        axios.put('http://localhost:5000/serviceBook/update/status/'+ id, newStatus )
            .then(res => {
    ...
    deleteService(id){
        axios.delete('http://localhost:5000/serviceBook/'+id)
            .then(res => console.log("Service deleted" + id))
    ...
    detailsList(){
        return this.state.details.map(currentDetails => {
            return <Row details={currentDetails} statusUpdate={this.updateStatus(currentDetails.status, currentDetails._id)} deleteService={this.deleteService} key = {currentDetails._id} />
        })
    }
...
const Row = (props) => {
    return (
        ...
        <Table.Cell><a href="#" onClick={() => props.deleteService(props.details._id)}>Delete</a></Table.Cell>
      </Table.Row>
    );
}